...
(fact "SimpleDB reads values as JSON"
  (let [db (db/newDB {} "test")]
    ...
    (provided
      (jdbc/query {} ["SELECT * FROM test WHERE id = ?" "1"]) => [{:id "1" :names "~json~[\"a\",\"b\"]"}])))
...
(fact "SimpleDB reads underscores in field names as dashes"
  (let [db (db/newDB {} "test")]
    ...
    (provided
      (jdbc/query {} ["SELECT * FROM test WHERE id = ?" "1"]) => [{:id "1" :allow_search true}])))

(fact "SimpleDB gets user views"
  (let [db (db/newDB {} "test")]
    (simpledb.db/get-user-view db "foo") => [{:id "1" :value {:id "1" :username "foo"}}]
    (provided
      (jdbc/query {} ["SELECT * FROM test WHERE username = ?" "foo"]) => [{:id "1" :username "foo"}])))
